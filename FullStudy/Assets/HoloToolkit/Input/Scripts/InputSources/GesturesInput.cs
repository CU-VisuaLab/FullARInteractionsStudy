...
namespace HoloToolkit.Unity.InputModule
{
    ...
    {
        // GesturesInput augmentation for logging
        public bool logging;
        ...
        private void OnTappedEvent(InteractionSourceKind source, int tapCount, Ray headRay)
        {
            if (logging)
            {
                if (GazeManager.Instance.HitObject != null) Debug.Log("Click " + GazeManager.Instance.HitObject.name + " at " + Time.time);
                else Debug.Log("Click Null Object at " + Time.time);
            }
