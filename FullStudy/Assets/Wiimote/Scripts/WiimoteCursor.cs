...
namespace HoloToolkit.Unity.InputModule
{
    ...
    /// </summary>
    public abstract class WiimoteCursor : MonoBehaviour, ICursor
    {
        /// <summary>
        ...
        protected virtual void RegisterManagers()
        {
            ...
            WiimoteInputManager.Instance.AddGlobalListener(gameObject);

            // Setup the cursor to be able to respond to input being globally enabled / disabled
            if (WiimoteInputManager.Instance.IsInputEnabled)
            {
                OnInputEnabled();
            }
            else
            ...
            WiimoteInputManager.Instance.InputEnabled += OnInputEnabled;
            WiimoteInputManager.Instance.InputDisabled += OnInputDisabled;
        }
        ...
        protected virtual void UnregisterManagers()
        {
            ...
            if (WiimoteInputManager.Instance != null)
            {
                WiimoteInputManager.Instance.RemoveGlobalListener(gameObject);
                WiimoteInputManager.Instance.InputEnabled -= OnInputEnabled;
                WiimoteInputManager.Instance.InputDisabled -= OnInputDisabled;
            }
        }
        ...
        protected virtual void UpdateWiimoteCursorTransform()
        {
            // Get the necessary info from the gaze source
            RaycastHit hitResult = gazeManager.HitInfo;
            GameObject newTargetedObject = gazeManager.HitObject;
